// Carga de dependencias usando el cliente de Supabase existente
import { createClient } from '../lib/supabase/server.js';

const supabase = createClient();

const dependenciasSQL = `
-- Carga completa de dependencias desde el CSV proporcionado (versión corregida)
...
('214', 'CUERPO BOMBEROS', 'CUERPO DE BOMBEROS CHÍA', 'descentralizado', NULL, 0, 214, true, NULL),
('215', 'DEFENSA CIVIL', 'DEFENSA CIVIL COLOMBIANA', 'descentralizado', NULL, 0, 215, true, NULL);
...
async function loadDependencias() {
  try {
    ...
    if (error) {
      console.log('Conexión básica exitosa, intentando ejecutar SQL...');
    }

    // Ejecutar el SQL directamente a través de fetch
    const response = await fetch('/api/admin/dependencias/import', {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      console.log('✅ Carga de dependencias completada exitosamente!');
      console.log('Resumen de la carga:', result);
    } else {
