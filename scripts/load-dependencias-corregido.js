...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/chia_municipal'
});
...
async function loadDependencias() {
  try {
    ...
    console.log('Ejecutando carga de dependencias...');
    const result = await client.query(dependenciasSQL);
    
    console.log('✅ Carga de dependencias completada exitosamente!');
    console.log('Resumen de la carga:');
    result.forEach((res, index) => {
      if (res.rows && res.rows.length > 0) {
        console.log(`Resultado ${index + 1}:`, res.rows);
      }
    });
